...
def write_x_indexes(output: Path, catalog: dict[str, object]) -> None:
    ...
    for _filename, xlfd, record in entries:
        add_alias(f'cadr-{safe_filename(record["name"])}', xlfd)
        if record["variant_of"] is None:
            add_alias(f'cadr-{safe_filename(record["runtime_name"])}', xlfd)
...
def main() -> int:
    ...
    try:
        ...
        write_x_indexes(output, catalog)
