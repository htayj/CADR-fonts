...
``2602eab2ef1bea4800312f71f9185e9261c6fa6c`` and adapted here for a pinned,
standalone, XLFD-producing build.
"""
...
# These authored source names compile to different resident FONT symbol names.
# The bindings are independently visible in the pinned public QFASLs; every
# other source-backed name currently retains its authored spelling.
RUNTIME_FONT_NAME_OVERRIDES = {
    "CM10": "CPT-CM10",
    "CM12": "CPT-CM12",
    "CPTFON": "CPTFONT",
}
...
def parse_ast(path: Path, words: list[int]) -> BitmapFont:
    ...
    if column_adjust != 0:
        raise SourceError(
            "nonzero AST column-position adjustment has no established CADR "
            "or BDF semantics"
        )
    glyphs = []
    source_metrics = {}
    for page_number, page in enumerate(pages[1:], 1):
        ...
        # rows are intentionally blank, not evidence of truncation.  Keep the
        # authored count here: the historical compiler separately computes
        # FONT-RASTER-HEIGHT as the greatest supplied row count.
        ...
        glyphs.append(
            Glyph(
                ...
                advance=advance,
                x_offset=x_offset,
                y_offset=baseline - supplied_row_count,
                rows=tuple(rows),
            )
        )
    ...
    raster_height = max(len(glyph.rows) for glyph in glyphs)
    glyphs = [
        Glyph(
            code=glyph.code,
            ...
            rows=glyph.rows + (0,) * (raster_height - len(glyph.rows)),
        )
        for glyph in glyphs
    ]
    return BitmapFont(
...
def parse_kst(path: Path, words: list[int]) -> BitmapFont:
    ...
    column_adjust = _signed((words[1] >> 27) & 0o777, 9)
    if column_adjust != 0:
        raise SourceError(
            "nonzero KST column-position adjustment has no established CADR "
            "or BDF semantics"
        )
...
def main() -> int:
    ...
    def emit_font(font: BitmapFont, source: Path) -> None:
        ...
        runtime_name = RUNTIME_FONT_NAME_OVERRIDES.get(logical_name, logical_name)
    ...
    catalog = {
        ...
        "runtime_name_override_count": len(RUNTIME_FONT_NAME_OVERRIDES),
        "bdf_policy": {
            "format": "BDF 2.1",
            "font_name": "complete 14-field XLFD",
            "font_kind": "one-bit source bitmap; no vector outlines or synthesized scaling",
            "encoding": "original CADR numeric code under Misc-FontSpecific",
            "nonrendering_placeholders": (
                "zero-advance, zero-width, no-ink source slots are retained in "
                "normalized JSON and omitted from installable BDF"
            ),
            "metric_mapping": {
                "DWIDTH": "source character width/advance",
                "BBX_x_offset": "negative source left-kern; this is a per-glyph bearing, not pair kerning",
                "FONT_ASCENT": "source baseline",
                "FONT_DESCENT": "source character height minus baseline",
            },
